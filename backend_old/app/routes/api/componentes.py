...
def get_componentes():
    ...
    try:
        ...
        if search:
            ...
            query = query.filter(
                db.or_(
                    ...
                    Componente.descripcion.ilike(search_term),
                    Componente.tipo.ilike(search_term)
                )
            )
        ...
        try:
            if sort_by == 'Nombre':
                if sort_order == 'desc':
                    query = query.order_by(Componente.nombre.desc())
                else:
        ...
        for comp in componentes:
            try:
                ...
                componentes_data.append({
                    'id': comp.id,
                    'nombre': comp.nombre or 'Sin nombre',
                    'descripcion': comp.descripcion,
                    'categoria': comp.tipo,
                    'precio_unitario': comp.precio or 0
                })
        
        response_data = {
            'success': True,
            'data': componentes_data,
            'total': total,
            'pagination': {
                'page': page,
                'pages': pages,
