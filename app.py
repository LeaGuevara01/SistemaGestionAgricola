...
# Machinery List
# This route retrieves the list of machines from the database and displays them in a table.
@app.route('/maquinas')
def lista_maquinas():
    conn = get_db_connection()
    maquinas = conn.execute('SELECT * FROM maquinas').fetchall()
...
# Machine Deletion
# This route allows users to delete a machine from the database.
@app.route('/maquinas/eliminar/<int:id>', methods=['POST', 'GET'])
def eliminar_maquina(id):
    conn = get_db_connection()
    try:
        cursor = conn.execute('DELETE FROM maquinas WHERE id = ?', (id,))
        conn.commit()
        if cursor.rowcount == 0:
            flash('La máquina no existe', 'warning')
        else:
            flash('Máquina eliminada correctamente', 'success')
    except Exception as e:
        conn.rollback()
        flash(f'Error al eliminar la máquina: {e}', 'danger')
...
if not WEATHER_API_KEY:
    raise RuntimeError("❌ La API KEY de OpenWeather no está definida")
...
def mapear_icono_clima(weather_id):
    ...
    iconos = {
        ...
        range(801, 805): "bi-cloud-sun"  # Nubes
    }
